...
app.get('/api/vehicles/:id', authMiddleware, async function(req, res) {
  ...
  vehicle.invoices = await dbAll('SELECT * FROM vehicle_invoices WHERE vehicle_id = ?', [req.params.id]);
  vehicle.materials = await dbAll('SELECT * FROM inbound_materials WHERE vehicle_id = ?', [req.params.id]);
  res.json(vehicle);
});
...
app.get('/api/grn/:grn_no', authMiddleware, async function(req, res) {
  ...
  grn.materials = await dbAll('SELECT * FROM inbound_materials WHERE vehicle_id = ?', [grn.vehicle_id]);
  var invRows = await dbAll('SELECT DISTINCT invoice_no FROM inbound_materials WHERE vehicle_id = ?', [grn.vehicle_id]);
  grn.invoices = [];
  for (var i = 0; i < invRows.length; i++) grn.invoices.push(invRows[i].invoice_no);
  res.json(grn);
});
...
app.get('/api/putaway/remaining/:grn_no', authMiddleware, async function(req, res) {
  ...
  for (var i = 0; i < inbound.length; i++) {
    var ib = inbound[i], paQty = 0;
    for (var j = 0; j < putaway.length; j++) {
      if (putaway[j].invoice_no === ib.invoice_no && putaway[j].material === ib.material) paQty += putaway[j].putaway_qty;
    }
    result.push({ invoice_no: ib.invoice_no, material: ib.material, ean: ib.ean || '', description: ib.description || '', inbound_qty: ib.qty, putaway_qty: paQty, remaining_qty: ib.qty - paQty, completed: ib.qty - paQty <= 0 });
  }
  res.json(result);
});
...
app.get('/api/putaway/difference/:grn_no', authMiddleware, async function(req, res) {
  ...
  var diff = [];
  for (var i = 0; i < inbound.length; i++) {
    var ib = inbound[i], paQty = 0, shortQty = 0;
    for (var j = 0; j < putaway.length; j++) {
      if (putaway[j].invoice_no === ib.invoice_no && putaway[j].material === ib.material) { paQty += putaway[j].putaway_qty; shortQty += putaway[j].short_qty; }
    }
...
app.post('/api/piv', authMiddleware, async function(req, res) {
  ...
  for (var i = 0; i < items.length; i++) {
    var it = items[i], pDate = it.date || new Date().toISOString().split('T')[0];
...
app.get('/api/location', authMiddleware, async function(req, res) {
  var page = parseInt(req.query.page) || 1, limit = parseInt(req.query.limit) || 200, offset = (page - 1) * limit;
...
app.get('/api/location/search', authMiddleware, async function(req, res) {
  ...
  var matList = materials.split(','), cleanList = [], placeholders = [], params = [];
  for (var i = 0; i < matList.length; i++) { var m = matList[i].trim(); if (m) { cleanList.push(m); placeholders.push('?'); params.push(m); } }
...
app.post('/api/location/bulk', authMiddleware, async function(req, res) {
  ...
  var today = new Date().toISOString().split('T')[0], count = 0, batchSize = 500;
  for (var b = 0; b < items.length; b += batchSize) {
    var batch = items.slice(b, b + batchSize), valuesSql = [], params = [];
    for (var i = 0; i < batch.length; i++) {
      ...
      if (qty <= 0) continue;
      valuesSql.push('(?,?,?,?,?,?,?,?,?,?)');
      params.push('manual', it.date || today, it.rack, it.ean || '', it.material, it.description || '', qty, it.packing || '', it.box_no || '', 1);
    }
    if (valuesSql.length === 0) continue;
    try { await db.execute({ sql: 'INSERT INTO location_data (source, date, rack, ean, material, description, qty, packing, box_no, active) VALUES ' + valuesSql.join(','), args: params }); count += valuesSql.length; } catch (e) { console.error('Batch error:', e.message); }
  }
...
app.post('/api/location/report', authMiddleware, async function(req, res) {
  ...
  for (var i = 0; i < location_ids.length; i++) { await dbRun('INSERT INTO location_report_items (report_id, location_data_id, action) VALUES (?, ?, ?)', [report.id, location_ids[i], 'none']); }
...
app.get('/api/location/report/:ho_no', authMiddleware, async function(req, res) {
  ...
  report.items = await dbAll('SELECT lri.id as report_item_id, lri.action, ld.* FROM location_report_items lri JOIN location_data ld ON lri.location_data_id = ld.id WHERE lri.report_id = ?', [report.id]);
  res.json(report);
});
...
app.put('/api/location/report/:ho_no/item/:item_id', authMiddleware, async function(req, res) {
  ...
  if (action === 'delete') {
    ...
    var minusQty = qty || reportItem.qty, newQty = Math.max(0, reportItem.qty - minusQty);
    if (newQty === 0) { await dbRun('UPDATE location_data SET active = 0 WHERE id = ?', [reportItem.location_data_id]); }
    else { await dbRun('UPDATE location_data SET qty = ? WHERE id = ?', [newQty, reportItem.location_data_id]); }
...
app.get('/api/location/audit', authMiddleware, async function(req, res) { res.json(await dbAll('SELECT * FROM location_audit ORDER BY id DESC')); });

app.get('/api/location/reports', authMiddleware, async function(req, res) { res.json(await dbAll('SELECT * FROM location_reports ORDER BY id DESC')); });

app.post('/api/location/pick', authMiddleware, async function(req, res) {
  if (!hasAccess(req.user, 'location')) return res.status(403).json({ error: 'No access' });
  var locationId = req.body.location_id, pickQty = parseFloat(req.body.pick_qty) || 0;
  ...
  if (newQty <= 0) { await dbRun('UPDATE location_data SET active = 0 WHERE id = ?', [locationId]); }
  else { await dbRun('UPDATE location_data SET qty = ? WHERE id = ?', [newQty, locationId]); }
...
app.get('/api/materials', authMiddleware, async function(req, res) {
  ...
  if (search) { res.json(await dbAll('SELECT * FROM material_master WHERE material LIKE ? OR description LIKE ? OR ean LIKE ? ORDER BY id DESC', ['%' + search + '%', '%' + search + '%', '%' + search + '%'])); }
...
app.post('/api/materials/bulk', authMiddleware, async function(req, res) {
  ...
  var count = 0, batchSize = 500;
  for (var b = 0; b < materials.length; b += batchSize) {
    var batch = materials.slice(b, b + batchSize), valuesSql = [], params = [];
    for (var i = 0; i < batch.length; i++) {
      var m = batch[i]; if (!m.material) continue;
      valuesSql.push('(?,?,?,?,?,?)'); params.push(m.material, m.description || '', m.div || '', m.ean || '', m.brand || '');
    }
    if (valuesSql.length === 0) continue;
    try { await db.execute({ sql: 'INSERT OR IGNORE INTO material_master (material, description, div, ean, brand) VALUES ' + valuesSql.join(','), args: params }); count += valuesSql.length; } catch (e) {}
  }
...
app.get('/api/materials/lookup/:ean', authMiddleware, async function(req, res) {
  res.json(await dbGet('SELECT * FROM material_master WHERE ean = ? OR material = ?', [req.params.ean, req.params.ean]) || null);
});

app.get('/api/bins', authMiddleware, async function(req, res) { res.json(await dbAll('SELECT * FROM bins ORDER BY rack, bin')); });
...
app.post('/api/bins/bulk', authMiddleware, async function(req, res) {
  ...
  var count = 0, batchSize = 500;
  for (var b = 0; b < bins.length; b += batchSize) {
    var batch = bins.slice(b, b + batchSize), valuesSql = [], params = [];
    for (var i = 0; i < batch.length; i++) {
      var bi = batch[i]; if (!bi.rack || !bi.bin) continue;
      valuesSql.push('(?,?,?)'); params.push(bi.rack, bi.bin, bi.capacity || 0);
    }
    if (valuesSql.length === 0) continue;
    try { await db.execute({ sql: 'INSERT OR IGNORE INTO bins (rack, bin, capacity) VALUES ' + valuesSql.join(','), args: params }); count += valuesSql.length; } catch (e) {}
  }
...
app.get('/api/bins/search/:material', authMiddleware, async function(req, res) {
  ...
  res.json({
    locations: await dbAll('SELECT * FROM location_data WHERE active = 1 AND material = ? ORDER BY rack', [mat]),
    putawayHistory: await dbAll('SELECT grn_no, rack, putaway_qty, created_at FROM putaway_records WHERE material = ? ORDER BY created_at DESC LIMIT 20', [mat]),
    pivHistory: await dbAll('SELECT rack, qty, created_at FROM piv_records WHERE material = ? ORDER BY created_at DESC LIMIT 20', [mat])
  });
});

app.get('/api/live-actions', authMiddleware, async function(req, res) { res.json(await dbAll('SELECT * FROM activity_log ORDER BY id DESC LIMIT 100')); });
...
async function startServer() {
  ...
  try {
    ...
    for (var i = 0; i < statements.length; i++) {
      ...
      if (s.length > 10) { try { await db.execute(s); } catch (e) {} }
    }
    ...
    for (var j = 0; j < statements2.length; j++) {
      ...
      if (s2.length > 10) { try { await db.execute(s2); } catch (e2) {} }
    }
